...
class _StudentsScreenState extends ConsumerState<StudentsScreen> {
  ...
  Widget _buildDataTable(List<Student> students) {
    ...
    // Compute allowed rows-per-page options and pick a rowsPerPage value
    // that is guaranteed to be inside availableRowsPerPage. This prevents
    // the PaginatedDataTable assertion: availableRowsPerPage.contains(rowsPerPage)
    final List<int> defaultOptions = isMobile ? [5] : [5, 10, 25, 50];
    final int desired = isMobile ? 5 : min(_rowsPerPage, students.length);

    final List<int> availableOptions = defaultOptions;
    final int rowsPerPageValue = availableOptions.contains(desired)
        ? desired
        : availableOptions.firstWhere((v) => v >= desired, orElse: () => availableOptions.last);
          ...
          Expanded(
            child: SingleChildScrollView(
              child: PaginatedDataTable(
                ...
                onRowsPerPageChanged: isMobile
                    ? null
                    : (value) {
                        if (value != null) setState(() => _rowsPerPage = value);
                      },
                availableRowsPerPage: availableOptions,
